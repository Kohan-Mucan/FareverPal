...
def poi_pixmap(cv, kind, label, size, done=False, tracked=False):
    ...
    if name:
        ...
        # Apply thin gold border to all uncollected chests as requested; pet
        # shop vendors render the gold $ marker from the minimap atlas.
        eff_accent = theme.GOLD if (not done and kind in ("chest", "petshop")) else accent
