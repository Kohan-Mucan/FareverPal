...
class _WrapLabel(QtWidgets.QLabel):
    """QLabel that wraps long text instead of eliding it, so the list row
    expands vertically and full names (dungeon / zone / chest) stay readable.
    Growth is purely vertical — the 2-column block never pushes wider than
    the panel, and PySide6's QLabel needs no manual paint work for wrapping."""
...
class DungeonListCell(QtWidgets.QFrame):
    ...
    def __init__(self, ui, dungeon: dict, key, parent=None):
        ...
        self._name_lbl = _WrapLabel(dungeon.get("boss_name") or icon_id or "?")
        self._name_lbl.setStyleSheet(f"color:{theme.TEXT}; font-size:14px; font-weight:bold;")
        # Let long names shrink + wrap (row grows vertically) instead of
        # forcing the row (and the 2-column block) wider than the panel
        # under the map.
